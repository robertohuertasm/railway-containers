...
{#if projectInfo}
	<div class="flex mb-4 items-center">
		...
		<button
			on:click={() => (showAddImageModal = true)}
			class="ml-auto text-gray-500 hover:text-gray-800 focus:outline>">+ ADD IMAGE</button
		>
	</div>
	...
	<div class="flex flex-wrap gap-8">
		{#if projectInfo.services.length}
			{#each projectInfo.services as service (service.id)}
				<ServiceItem {service} on:deleteService={onDeleteService} />
			{/each}
		{:else}
